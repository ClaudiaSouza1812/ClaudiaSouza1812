...
def main():
    ...
    msg02 = '--> With the dot and extension apart, the file name must have 12 characters the most, without spaces and any other signals except by the dot, numbers are allowed.'
    msg03 = '--> The valid extensions are: '
    # print the constraints and the extensions acceptables
    print(msg01 + '\n' + msg02 + '\n' + msg03 + '\n' + ', '.join(extlist))
    ...
    while True:
        ...
        if '.' not in asw01:
            ...
            continue
        # create a list with the splitted input
        name = asw01.split(".", maxsplit=2)
        # check the name length
        if len(name[0]) < 1 or len(name[0]) > 12:
            print('--> The file name must have between 1 and 12 characters with the dot in the right place')
            continue
        ...
        if all(c in acpt for c in asw01) and ext in extlist:
            ...
            print('--> Digit a valid file name (only letters, numbers and a dot followed by the name) with the valid extensions: ', ', '.join(extlist))
